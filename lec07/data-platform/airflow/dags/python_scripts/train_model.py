...
def process_iris_data(**kwargs):
    ...
    df.drop(
        ...
        axis=1,
        inplace=True,
        errors="ignore",  # Use errors='ignore' to handle columns that might not exist
    )
    ...
    print(
        f"Initial model - Training accuracy: {train_score:.4f}, Test accuracy: {test_score:.4f}"
    )
    ...
    print(
        f"Top 5 features model - Training accuracy: {train_score_top5:.4f}, Test accuracy: {test_score_top5:.4f}"
    )
